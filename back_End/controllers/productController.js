...
const createProduct = async (req, res) => {
  try {
    ...
    let discountPrice = Math.round(price * (1 - discountPercentage / 100));
...
const imany = async (req, res) => {
  try {
    const result = await productModel.productModel.insertMany(products);
    if (result) {
      return res.status(200).send({ message: "success", data: result });
    }
    return res.status(500).send({ message: "Something went wrong" });
...
const getProudct = async (req, res) => {
  ...
  let query = productModel.getProduct(condition);
  let totalProductsQuery = productModel.getProduct(condition);
...
const UpdateByPatch = async (req, res) => {
  try {
    ...
    const result = await productModel.PatchUpdate(id, {
      ...req.body,
      discountPrice: discountPrice,
    });
    if (result) {
